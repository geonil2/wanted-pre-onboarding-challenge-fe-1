...
const ToDoForm = ({
  ...
  handleClickUpdateButton?: (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => void,
  ...
  return (
    <form className='py-4'>
      {/*{formType === 'addToDoListForm' || formType === 'selectedToDoListForm' && toDoList.title ?*/}
        <>
          <p className="text-2xl text-[#195dae] font-bold text-center">
            {formType === 'addToDoListForm' ? 'Add List' : 'Detail List'}
          </p>
          <div className="flex flex-col">
            ...
            {formType === 'selectedToDoListForm' ?
              <div className="mt-2">
                {/*<button onClick={handleClickUpdateButton} className="mr-2">*/}
              ...
              >{formType === 'addToDoListForm' ? 'Save' : 'Update'}</button>
            {/*:*/}
            ...
            {/*    <button*/}
            {/*      name="update"*/}
            {/*      // onClick={() => dispatch(ToDoService.updateToDo({ ...toDoList, id: data.id }))}*/}
            {/*      onClick={submitToDoForm}*/}
            {/*      className="w-[49%] pointer-events-auto rounded-md text-center font-medium shadow-sm ring-1 ring-slate-700/10 py-2 px-4 mt-4 hover:bg-slate-50"*/}
            {/*    >Update*/}
            {/*    </button>*/}
